...
app.get("/", (req, res) => {
  res.json({
    message: "CoastalWatch Backend API",
    version: "1.0.0",
    endpoints: {
      health: "/health",
      auth: "/api/auth",
...
if (!process.env.MONGO_URI && !process.env.MONGODB_URI) {
  ...
  console.error(
    "MONGO_URI or MONGODB_URI env not found. Expected mg.env at:",
    path.join(__dirname, "mg.env"),
  );
}
...
connectDB(process.env.MONGO_URI || process.env.MONGODB_URI)
  ...
  .catch((err) => {
    ...
    // For integration testing, start server anyway without DB
    console.warn("Starting server without MongoDB for integration testing...");
    server.listen(PORT, () => {
      // eslint-disable-next-line no-console
      console.log(`Server listening on port ${PORT} (MongoDB unavailable)`);
    });
  });
